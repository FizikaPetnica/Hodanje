...
W = 15 * np.random.rand(20, 8, 4) - 7.5
M = np.zeros((4,))
ocena = np.zeros((20,), dtype=[('x', int),('y', int)])

world=world(gravity=(0,-9.81),doSleep=True)

ground=world.CreateStaticBody(position=(30,2))
box=ground.CreatePolygonFixture(box=(30,3), density=1, friction=1)
...
for i in range(50):
    ...
    for j in range(20):
        ...
        torso.position = (5,hi)
        torso.angle = 0
        left_leg.position = (5+1.4*math.sin(ang1),hi-2-1.4*math.cos(ang1))
        left_leg.angle = ang1
        right_leg.position = (5+1.4*math.sin(ang2),hi-2-1.4*math.cos(ang2))
        right_leg.angle= ang2
        left_underknee.position = (5+2.8*math.sin(ang1)+1.4*math.sin(ang3),hi-2-2.8*math.cos(ang1)-1.4*math.cos(ang3))
        left_underknee.angle = ang3
        right_underknee.position = (5+2.8*math.sin(ang2)+1.4*math.sin(ang4),hi-2-2.8*math.cos(ang2)-1.4*math.cos(ang4))
        right_underknee.angle = ang4
        ...
        while running:
            for event in pygame.event.get():
                if event.type==QUIT or (event.type==KEYDOWN and event.key==K_ESCAPE):
                    running=False
            if cnt==600 or torso.worldCenter.y<6:
                running=False
            cnt += 1
            ...
            # zglob.angle - read only
            # zglob.speed - read only
            # zglob.GetMotorTorque - read only
            # zglob.motorspeed - read-write
            # zglob.maxMotorTorque - write only
            ...
            M = np.dot(polozaj, W[j,:,:])
##            for k5 in range(4):
##                if np.absolute(M[k5])>20:
##                    M[k5] = 20*np.sign(M[k5])
            if M[0]>0:
            ...
            #kicma tri dela, ruke dva dela, vrat i glava, neuronska, genet, 
            
            #pygame.display.flip()
            #clock.tick(TARGET_FPS)

        ocena[j] = (torso.worldCenter.x+2*(torso.worldCenter.y-5), j)
        pygame.quit()

    cnt1 = 0
    ocena.sort(kind = 'heapsort', order='x')
    for k in range(5):
        for k1 in range(8):
            for k2 in range(4):
                if np.random.random()>0.5:
                    W[ocena[k][1]][k1][k2]=W[ocena[10+k][1]][k1][k2]
                else:
                    W[ocena[k][1]][k1][k2]=W[ocena[19-k][1]][k1][k2]
                if np.random.random()<0.005:
                    W[ocena[k][1]][k1][k2]=np.random.random()*15-7.5
    for k in range(4):
        for k0 in range(4-k):
            for k1 in range(8):
                for k2 in range(4):
                    if np.random.random()>0.5:
                        W[ocena[5+cnt1][1]][k1][k2]=W[ocena[15+k][1]][k1][k2]
                    else:
                        W[ocena[5+cnt1][1]][k1][k2]=W[ocena[19-k0][1]][k1][k2]
                    if np.random.random()<0.005:
                        W[ocena[5+cnt1][1]][k1][k2]=np.random.random()*15-7.5
            cnt1 += 1
    print(i)
